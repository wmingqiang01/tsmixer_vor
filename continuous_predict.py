...
def predict_multiple_samples(model_path, data_dir):
    """
    持续预测多个样本文件夹并通过print输出结果
    
    参数:
    model_path: ONNX模型文件路径
    data_dir: 包含多个样本文件夹的目录路径
    """
    input_length = 24
    ...
    # 获取所有样本文件夹
    sample_folders = sorted([f for f in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, f))])
    
    if not sample_folders:
        print(f"数据目录 {data_dir} 中没有样本文件夹")
        return

    # 遍历每个样本文件夹
    for folder in sample_folders:
        folder_path = os.path.join(data_dir, folder)
        print(f"正在处理样本: {folder_path}")
        
        X_test, metadata = process_folder_for_prediction(folder_path, input_length)

        if X_test is None:
            print(f"样本 {folder} 处理失败，跳过")
            continue
        ...
        for i, pred in enumerate(y_pred):
            meta = metadata[i]
            print(f"station: {meta['station']}, 日期: {meta['year']}-{meta['month']}-{meta['day']:}")
            print(f"经度: {meta['lon']:.4f}, 纬度: {meta['lat']:.4f}")
            print(f"预测标签: {pred}")
            print("-----------------")
...
if __name__ == '__main__':
    ...
    data_dir = "test_data/samples"  # 包含多个样本文件夹的目录
    
    predict_multiple_samples(model_path, data_dir)